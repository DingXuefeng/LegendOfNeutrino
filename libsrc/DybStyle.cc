...
TStyle* DybStyle::get(){
  if(!m_dybStyle){
    ...
    Double_t tsize=0.04; //should be set between 0.04-0.06, is in units of "% of pad"
    ...
    // By Ding Xuefeng
    // turn on x y grids
    //m_dybStyle->SetPadGridX(1);
    //m_dybStyle->SetPadGridY(1);
...
void DybStyle::set()
{
  ...
  gROOT->ForceStyle();
  //gStyle->ls();
  //std::cout<<gStyle->GetName()<<std::endl;
}
